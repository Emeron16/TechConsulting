...
(per NRG_Energy_Architecture_Deep_Dive.md §2.11's AWS CloudTrail node and
§3's "not GxP but still a regulated consumer-facing industry" framing).
Each row's record_hash covers its own content plus the previous row's
hash, so altering any past row breaks every hash after it. The
audit_log table's UPDATE/DELETE triggers (db/init/01_schema.sql) enforce
insert-only at the DB level; this module enforces the chain's content
integrity at the application level. Ported near-verbatim from
copilot-demo's mcp_servers/audit.py -- the hash-chaining algorithm itself
is domain-agnostic.

Written on two event types here: document_ingested (nrg_chains/ingest.py,
one entry per publish) and answer_sampled_reviewed (nrg_app_render.py's
Compliance Review tab / scripts/review.py, one entry per review
annotation) -- unlike copilot-demo, there is no e-signature/approval event
type, since there's no approval gate to sign off on here.
"""
import hashlib
import json
